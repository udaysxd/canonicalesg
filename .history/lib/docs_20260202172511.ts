export const DOCS = {
  cerm: {
    version: "v1",
    pages: [
      { slug: "design-principles", file: "01-design-principles.md", title: "Design Principles" },
      { slug: "architecture", file: "02-architecture.md", title: "Architecture" },
      { slug: "reference", file: "03-reference.md", title: "Reference Model" }
    ]
  },
  cdi: {
    version: "v1",
    pages: [
      { slug: "design-principles", file: "01-design-principles.md", title: "Design Principles" },
      { slug: "architecture", file: "02-architecture.md", title: "Architecture" },
      { slug: "taxonomy", file: "03-taxonomy.md", title: "Climate Taxonomy" }
    ]
  },
  cmp: {
    version: "v1",
    pages: [
      { slug: "design-principles", file: "01-design-principles.md", title: "Design Principles" },
      { slug: "architecture", file: "02-architecture.md", title: "Architecture" },
      { slug: "specification", file: "03-content-specification.md", title: "Specification" },
      { slug: "how-to-build", file: "how-to-build.md", title: "How to Build a CMP" },
      { slug: "registry", file: "registry.md", title: "CMP Registry" }
    ]
  }
};
