...
export default async function WhitepaperPage() {
  ...
  // Split HTML after the H1 title to insert author block
  const h1EndIndex = html.indexOf('</h1>');
  const beforeH1 = h1EndIndex !== -1 ? html.slice(0, h1EndIndex + 5) : '';
  const afterH1 = h1EndIndex !== -1 ? html.slice(h1EndIndex + 5) : html;

  const authorBlock = `
    <section class="mb-6 border-b border-[#e5e5e5] pb-4">
      <div class="flex items-start gap-3">
        ...
        />
        <div>
          <p class="text-[#111] font-semibold text-[1rem] leading-tight">
            Uday Singh
          </p>
          <p class="text-[#444] text-[0.75rem] mb-1">
            Founding Architect, Canonical ESG
          </p>
          <p class="text-[#555] text-[0.75rem] leading-snug max-w-[560px]">
            Uday Singh leads the development of Canonical ESG, an independent technical initiative focused on semantic infrastructure for sustainability disclosure. His work centres on ESG data architecture, cross-framework reporting alignment, and structured implementation models designed to enhance regulatory coherence and long-term interoperability.
          </p>
        </div>
      </div>
    </section>
  ...
  const htmlWithAuthor = beforeH1 + authorBlock + afterH1;
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        <div>
          ...
          <article
            ...
            dangerouslySetInnerHTML={{ __html: htmlWithAuthor }}
          />
        </div>
      </main>
    </>
